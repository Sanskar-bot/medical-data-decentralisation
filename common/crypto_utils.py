...
def generate_rsa_keypair(key_size: int = 4096):
    # Medical records require long-term confidentiality; 4096-bit provides a wider
    # margin against future attacks (e.g., advances in factoring algorithms).
    # Note: existing keys are NOT retroactively changed — re-registration generates
    # a new keypair with the updated size.
...
def rsa_hybrid_decrypt(priv, blob_b64: str) -> bytes:
    ...
    key_size = priv.key_size // 8   # 256 for RSA-2048, 512 for RSA-4096
...
def derive_kek_from_password(password: str, salt: bytes = None, iterations: int = 200_000):
    """
    DEPRECATED: Use derive_kek_argon2() for new key derivation.
    Kept for backward compatibility with existing wrapped keys that used PBKDF2.
    """
    if salt is None:
        salt = os.urandom(16)
    kdf = PBKDF2HMAC(
...
def derive_kek_argon2(password: str, salt: bytes = None):
    """
    Derive a 32-byte Key-Encryption-Key from a password using Argon2id.

    Preferred over derive_kek_from_password() for new keys — Argon2id is
    memory-hard and resistant to GPU/ASIC brute-force attacks.

    Parameters (tuned for interactive login latency ~0.5s on 2024 hardware):
